...
interface ContextEditorProps {
    ...
    /** When set, that environment's column is highlighted (e.g. the env you're querying). */
    highlightEnvironment?: string;
}
...
export default function ContextEditor({
    ...
    const handleRenameProfile = (name: string) => {
        ...
        setDirty(true);
        setDraft((prev) =>
            prev.map((p) => (p.id === selectedProfile.id ? { ...p, name } : p)),
        );
    };
    ...
    const handleSave = async () => {
        ...
        for (const profile of draft) {
            if (!PROFILE_ID_REGEX.test(profile.id) || !profile.name.trim()) {
                setFeedback(
                    `Profile "${profile.name || profile.id}" needs a valid id and name.`,
                );
                return;
            }
        }
